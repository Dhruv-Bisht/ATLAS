...
function daysLeft(deadline:string){return Math.max(0,Math.ceil((new Date(deadline).getTime()-Date.now())/86400000));}
...
export default function AtlasDashboard(){
  ...
  const filtered=useMemo(()=>items.filter(x=>{
    ...
    return (type==='ALL'||x.type===type)&&(country==='ALL'||x.country===country)&&(field==='ALL'||x.field===field)&&(workMode==='ALL'||(x.workMode??'').toLowerCase().includes(workMode.toLowerCase()))&&(experience==='ALL'||(x.experienceRequired??'').includes(experience))&&(deadline==='ALL'||(deadline==='7'&&d<=7)||(deadline==='30'&&d<=30))&&matchQ;
  ...
  return <main className="atlas-shell">
    ...
    <section className="workspace">
      ...
      <aside className="details-panel">{selected ? <div className="detail-card"><button className="close" onClick={()=>setSelected(null)}>×</button><div className="pin-title"><span style={{color:typeMeta[selected.type].color}}>◆</span><div><h1>{selected.title}</h1><h2>{selected.organization}</h2></div></div><span className="type-pill" style={{borderColor:typeMeta[selected.type].color,color:typeMeta[selected.type].color}}>{typeMeta[selected.type].label.slice(0,-1).toUpperCase()}</span><div className="detail-lines"><p>⌖ <b>{selected.city ? `${selected.city}, `:''}{selected.region ? `${selected.region}, `:''}{selected.country}</b></p><p>▣ <b>{selected.workMode ?? 'Not specified'}</b></p><p>♙ <b>{selected.experienceRequired ?? 'Not specified'}</b></p>{selected.salary&&<p>◉ <b>{selected.salary} {selected.currency ?? ''}</b></p>}<p>▣ <b>Deadline: {new Date(selected.deadline).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}</b> <span className="days">{daysLeft(selected.deadline)} days left</span></p></div><p className="description">{selected.description}</p><a className="apply" href={selected.applicationUrl} target="_blank" rel="noreferrer">View Opportunity ↗</a><a className="company-link" href={selected.sourceUrl ?? selected.applicationUrl} target="_blank" rel="noreferrer">↗ &nbsp; Visit Company / Official Website</a>{selected.verified&&<div className="verified">✓ Verified official source</div>}</div>:<div className="empty-detail">SELECT A PIN<br/><small>Choose an opportunity on the globe.</small></div>}</aside>
    </section>
